...
def delete_incident(event_id, incident_id):
    ...
    db.session.commit()
    return jsonify({'status': 'success', 'message': 'Incidente eliminado'})
...
def add_assignment(event_id, incident_id):
    ...
    if not data or 'asignado_a' not in data:
        return jsonify({'status': 'error', 'message': 'El campo asignado_a es requerido'}), 400
    assignment = IncidentAssignment(
        incident_id=incident_id,
        asignado_a=data['asignado_a'],
        estado='pre-avisado',
        hora_preavisado=datetime.now()
    )
    db.session.add(assignment)
    db.session.commit()
    return jsonify({'status': 'success', 'assignment': assignment_to_dict(assignment)})
...
def update_assignment(event_id, incident_id, assignment_id):
    ...
    if 'estado' in data:
        ...
        if data['estado'] == 'avisado y en camino':
            assignment.hora_avisado = datetime.now()
            assignment.hora_en_camino = datetime.now()
        elif data['estado'] == 'en el lugar':
            assignment.hora_en_lugar = datetime.now()
        elif data['estado'] == 'desasignado':
            assignment.hora_desasignado = datetime.now()
    db.session.commit()
    return jsonify({'status': 'success', 'assignment': assignment_to_dict(assignment)})
